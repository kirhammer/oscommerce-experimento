...
| Conversión multimoneda (`currencies->display_price`, tabla `currencies`) | product_listing.php:116-119 | DATO | **API + SPA/Embed** | parámetro `?currency=` en R1/R2 (tasa + reglas de formato de la tabla `currencies`), endpoint `GET /api/currencies`, selector en la SPA; el embed sigue la moneda de sesión del legado vía `data-currency` |
| Precio con impuestos (`tep_get_tax_rate`) | product_listing.php:116-119 | PRESENTACIÓN | Fuera de alcance | el motor de impuestos es transversal a la tienda (otro requisito); la API entrega `tax_class_id` para calcularlo |
